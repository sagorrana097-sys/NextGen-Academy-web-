...
export function formatAcademicBadge(board, year, qType = 'CQ') {
...
export function parseMultiBoardPrompt(rawPrompt, defaultType = 'CQ') {
  ...
  const boardPattern = boardNames.map(b => b.replace(/[.*+?^${}()|[\]\\]/g, '\\export default function AICQGeneratorModal({')).join('|');
              ...
              {selectedSourceMaterialId && (
                <div className="p-2.5 rounded-xl bg-emerald-900/40 border border-emerald-500/40 text-[11px] text-emerald-200 flex items-center justify-between">
                  ...
                  <span className="shrink-0 font-mono text-[10px] bg-emerald-950 px-2 py-0.5 rounded border border-emerald-700">
                    {filteredSourceMaterials.find(m => String(m.id) === String(selectedSourceMaterialId))?.content_text?.length || 0} Chars
                  </span>
                </div>
              )}
            </div>
